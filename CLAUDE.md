...
17. **`createOrder` is the single authoritative gate — every admin control is re-enforced server-side.** `lib/orders/create.ts` does not trust any pricing or state value from the client. In order it: (a) calls `getOperations()` (`store_open` / `cod_enabled` / `closed_message`, cached under `OPERATIONS_TAG`) and rejects if the kitchen is paused; (b) calls `getHours()` and rejects when `delivery_date`'s weekday is not in `hours.days` or the same-day cutoff has passed (date math runs in `Europe/London`); (c) re-runs `matchZoneByPostcode` and rejects if the postcode isn't in any active zone; (d) selects `id, name, price_gbp, is_available_today, is_hidden, archived_at, is_cod_eligible, variants, addons` for every line's UUID and rejects if any item is missing/hidden/archived/unavailable; (e) recomputes each line's `unitPriceGbp` from the DB `price_gbp` + DB variant/addon `price_delta_gbp` looked up by chosen label (the client's claimed deltas are discarded — a stale or tampered chosen label returns "please re-customise"); (f) recomputes `subtotal` and `total` from server-trusted line totals and re-checks `zone.minOrderGbp`; (g) re-checks COD against `operations.codEnabled && zone.allowsCod && every(line.isCodEligible)`. The order_items insert stores the server-trusted price, not the client's. **Do not** add client-derived price or availability values back into the insert; the helpers `validateDeliveryDate` and `resolveUnitPrice` at the bottom of `create.ts` are the contract. The new `lib/data/operations.ts` fetcher is paired with `updateSetting` (in `lib/admin/catalogActions.ts`) which `revalidateTag(OPERATIONS_TAG, 'default')` on `store_open` / `cod_enabled` / `closed_message` writes — any future setting consumed by the customer must get the same tag-pair treatment. `/checkout` reads `operations.storeOpen` and renders a "kitchen paused" card instead of the form when it's false; `CheckoutForm` takes `openDays` + `sameDayCutoff` from the same admin source so the date picker can't offer a closed day.
