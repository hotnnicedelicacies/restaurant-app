...
lib/
  ...
  data/
    ...
    trailer.ts              — getTrailer(): the food trailer's venue/postcode/days/hours from `settings.trailer`, tag 'trailer'. `enabled=false` hides every mention site-wide
...
components/
  ...
  home/                     — HeritageHero, DayBand (async, reads hours), BillOfFare wrappers, HygieneSection, KitchenStory (portrait/landscape image, external link), HowItWorks (async), DeliveryAreas, CtaBand
  trailer/                  — TrailerSection (home walnut band: photo + where/when/how listing) and TrailerCard (contact page). Both async, read getTrailer(), render null when disabled
...
15. **Account `closeAccount` anonymises rather than deletes — and is blocked while orders are in progress.** `closeAccount` in `lib/account/profile.ts` first calls `hasOpenOrders(userId)` which checks for any order whose `status` is `received`, `preparing`, or `on_its_way`. If one exists we return a clear error and the UI (`components/account/CloseAccountCard.tsx`) shows a danger banner + disables the close button — the kitchen and driver still need name/phone/address to perform the contract (UK GDPR Art. 6(1)(b)). The `/account` page passes a derived `hasOpenOrders` prop so the banner is visible before the user even tries. Once cleared, closing the account scrubs PII on `profiles` + on historical `orders` (display_name, customer_email, customer_phone, delivery_line1/2) and hard-deletes `addresses`, then calls `auth.admin.deleteUser`. The order row itself stays — UK tax record-keeping needs the line items + amounts for 6 years. Terms section 2 documents this; keep code + terms in sync.

16. **Delivery zones + per-meal COD are admin-controlled and authoritative.** `delivery_zones` (rows + postcode arrays + `base_fee_gbp` + `min_order_gbp` + `allows_cod`) drives **every** customer-facing delivery surface — never hardcode the fee or area list. The fetcher `getActiveZones()` in `lib/data/zones.ts` is wrapped in `unstable_cache` with `ZONES_TAG`; admin mutations must `revalidateTag(ZONES_TAG, 'default')`. Customer surfaces wired to it: home `<DeliveryAreas>`, contact `<DeliveryAreas>`, layout JSON-LD `areaServed`, menu CtaBand subtitle, cart "from £X" indicator, checkout `/api/zones/check`. The cart can only show a *from* fee — the exact fee is bound at checkout once postcode is known. Per-meal `menu_items.is_cod_eligible` is mirrored onto each `CartLine` so the checkout can disable the COD radio with a clear "Not available for: X" message. Cart store version bumped to v3 after adding the field; older persisted carts wipe on load. Anywhere you'd reach for `siteConfig.delivery.areas` / `pricing.middlesbrough` for a customer-facing surface — pull from `getActiveZones()` instead with siteConfig as a *cold-cache fallback only*.

17. **`createOrder` is the single authoritative gate — every admin control is re-enforced server-side.** `lib/orders/create.ts` does not trust any pricing or state value from the client. In order it: (a) calls `getOperations()` (`store_open` / `cod_enabled` / `closed_message` / `global_min_order_gbp` / `default_prep_time_*`, cached under `OPERATIONS_TAG`) and rejects if the kitchen is paused; (b) calls `getHours()` and rejects when `delivery_date`'s weekday is not in `hours.days` or the same-day cutoff has passed (date math runs in `Europe/London`); (c) re-runs `matchZoneByPostcode` and rejects if the postcode isn't in any active zone OR if the zone is `is_quoted = true` (quote-only zones can't self-serve; bounce to WhatsApp); (d) selects `id, name, price_gbp, is_available_today, is_hidden, archived_at, is_cod_eligible, variants, addons` for every line's UUID and rejects if any item is missing/hidden/archived/unavailable; (e) recomputes each line's `unitPriceGbp` from the DB `price_gbp` + DB variant/addon `price_delta_gbp` looked up by chosen label (the client's claimed deltas are discarded — a stale or tampered chosen label returns "please re-customise"); (f) recomputes `subtotal` and `total` from server-trusted line totals and re-checks `max(zone.minOrderGbp, operations.globalMinOrderGbp)` (global floor wins when higher); (g) re-checks COD against `operations.codEnabled && zone.allowsCod && every(line.isCodEligible)`. The order_items insert stores the server-trusted price, not the client's. **Do not** add client-derived price or availability values back into the insert; the helpers `validateDeliveryDate` and `resolveUnitPrice` at the bottom of `create.ts` are the contract. `updateSetting` (in `lib/admin/catalogActions.ts`) is the one place that fans out `revalidateTag` calls for every settings key admins can edit — `OPERATIONS_TAG`, `HOURS_TAG`, `EMAIL_CONFIG_TAG`, `CONTACT_TAG`, `TRAILER_TAG`. Any new setting consumed by the customer must add a `case` to that fan-out. `/checkout` reads `operations.storeOpen` and renders a "kitchen paused" card instead of the form when it's false; `CheckoutForm` derives the date picker from `hours.days` + `hours.sameDayCutoff` and the delivery-window list from `buildWindows(hours.open, hours.close)`.

18. **siteConfig.ts is brand-static only — every business value comes from a DB fetcher.** No `contact.*`, `delivery.*`, `hours.*`, or `email.*` lives in siteConfig anymore. Customer-facing surfaces pull from fetchers in `lib/data/*` (`getContact`, `getHours`, `getActiveZones`, `getEmailConfig`, `getOperations`, `getTrailer`), each cached with its own tag. The trailer (venue, postcode, days, hours, map link, on/off) is the `trailer` settings row, edited at `/admin/settings#trailer`; its consumers are home `<TrailerSection>`, contact `<TrailerCard>`, the About page block, the footer Hours column, and the root JSON-LD `location`. Photos live in `assets/trailer/` (real photos only — never the AI render or the street flyers). The fetchers carry their own deploy-time defaults inline so siteConfig doesn't double as a "fallback business value" file. Customer pages with WhatsApp/phone/email links (`/`, `/menu`, `/contact`, `/checkout`, `/track/[ref]`, `/receipt/[ref]`, `/cookie-notice`, `/privacy`, `/refund-policy`, `/terms`) are async server components calling `getContact()`; the JSON-LD in `app/layout.tsx` does the same. `<SiteFooter>` is async and reads `getHours() + getContact()`. `<CtaBand>` is async and reads `getContact()` for channel links. `<ContactForm>` (client) takes `whatsappNumber` as a prop. `<CheckoutForm>` (client) takes `whatsappNumber` for its "outside delivery area" / "quote-only" WhatsApp escape hatches. The Resend `sendEmail` consumes `getEmailConfig().fromHeader`/`replyTo`; admin email notifications go to `getEmailConfig().notificationTo`. Email templates (`orderConfirmationEmail`, `statusUpdateEmail`, `cancellationEmail`, `welcomeEmail`) take an `EmailContext` `{ contactEmail, contactWhatsapp }` rather than reading siteConfig — every caller is responsible for fetching it. Adding a hardcoded business value to siteConfig will introduce drift the moment the admin updates the equivalent setting — **don't**. Brand-static fields that DO live in siteConfig: `name`, `shortName`, `tagline`, `domain`, `description`, `social`, `foodHygiene`, `voice`, `routes`.
...
- **Baseline:** the first three migrations were applied by pasting `setup.sql`, so a fresh ledger starts empty. The workflow self-heals: when the remote ledger has zero rows it runs `migration repair --status applied` for exactly `20260511000001 20260511000002 20260511000003` (ledger rows only, no SQL) before pushing. The `mark_applied` dispatch input remains for manual ledger surgery — a run with it set only repairs and never pushes. Note: GitHub's "Re-run jobs" replays the original event with no input form; to use inputs you need the "Run workflow" dropdown on the workflow page.
