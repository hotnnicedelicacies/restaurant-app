...
export default function CartContents() {
  ...
  if (!mounted) {
    return (
      <div className="h-[400px] animate-pulse rounded-[2px] bg-[--color-cream-soft]" aria-label="Loading basket" />
    );
  }
  ...
  return (
    <div className="grid items-start gap-[clamp(32px,5vw,64px)] md:grid-cols-[1.4fr_1fr]">
      ...
      <main className="min-w-0">
        ...
        <div className="mt-7 flex flex-wrap gap-4 border-t border-[--color-border] pt-6">
      ...
      <aside className="sticky top-[92px]">
        <div className="rounded-[2px] border border-[--color-border] bg-[--color-cream] p-6">
          <header className="mb-4 flex items-baseline justify-between gap-3 border-b border-[--color-border] pb-3.5">
            <h2 className="m-0 font-serif text-[22px] font-medium tracking-[-0.005em] text-[--color-walnut]">
          ...
          <div className="mb-4 flex flex-col gap-2">
            ...
            <SummaryRow
              label={
                <span>
                  Delivery <em className="italic text-[--color-ink-muted]">· Middlesbrough</em>
                </span>
              }
          ...
          <Link
            ...
            className="block w-full rounded-[2px] border-0 bg-[--color-walnut] px-5 py-4 text-center font-serif text-[15px] font-semibold uppercase tracking-[0.16em] text-[--color-cream] [font-variant:small-caps] transition-colors hover:bg-[--color-bronze-deep]"
          >
            Proceed to checkout
          </Link>

          <div className="mt-4 text-center font-serif text-[12px] italic leading-[1.5] text-[--color-ink-muted]">
...
function CartRow({
  ...
  return (
    <article className={`grid grid-cols-[88px_1fr] gap-[14px] py-5 ${divider ? 'border-b border-[--color-border]' : ''} sm:grid-cols-[120px_1fr] sm:gap-5 sm:py-6`}>
      <Image
        src={line.imageSrc}
        alt={line.name}
        width={240}
      ...
      <div className="flex min-w-0 flex-col gap-2">
        <div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between sm:gap-4">
          <h3 className="m-0 font-serif text-[17px] font-medium tracking-[-0.005em] text-[--color-walnut] sm:text-[20px]">
            {line.name}
          </h3>
          <span className="whitespace-nowrap font-serif text-[16px] font-semibold text-[--color-walnut] sm:text-[18px]">
            {formatGBP(lineTotal)}
          </span>
        </div>

        {variantParts.length > 0 || addonsLine ? (
          <p className="m-0 font-serif text-[14px] italic leading-[1.45] text-[--color-ink-muted]">
            {variantParts.map((p, i) => (
              <span key={p}>
                {i > 0 && <span className="text-[--color-bronze]"> · </span>}
                <b className="font-medium not-italic text-[--color-walnut]">{p.split(': ')[0]}:</b>{' '}
                {p.split(': ')[1]}
              </span>
            ))}
            {addonsLine && (
              <>
                ...
                <b className="font-medium not-italic text-[--color-walnut]">Add-ons:</b> {addonsLine}
              </>
            )}
          </p>
        ) : null}
        ...
        {line.specialInstructions && (
          <p className="m-0 mt-1 border-l-2 border-[--color-border] pl-3 font-serif text-[13px] italic leading-[1.5] text-[--color-ink-muted]">
            "{line.specialInstructions}"
          </p>
        )}
        ...
        <div className="mt-2 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3.5">
            <div className="inline-flex items-center rounded-[2px] border border-[--color-border]" role="group" aria-label="Quantity">
              <button
                ...
                className="h-9 w-9 cursor-pointer font-serif text-[18px] text-[--color-walnut] transition-colors hover:bg-[--color-cream-soft]"
              >
                −
              </button>
              <span className="w-9 text-center font-serif text-[15px] font-medium text-[--color-walnut]">
                {line.quantity}
              </span>
              <button
                type="button"
          ...
          <button
            type="button"
            onClick={onRemove}
            className="border-0 border-b border-transparent bg-transparent pb-px font-serif text-[13px] italic text-[--color-ink-muted] transition-colors hover:border-[#8B2A1A] hover:text-[#8B2A1A]"
          >
            Remove
          </button>
        </div>
...
function SummaryRow({
  ...
  return (
    <div
      className={`flex justify-between font-serif text-[15px] text-[--color-walnut] ${
        muted ? 'italic text-[--color-ink-muted]' : ''
      } ${grand ? 'mt-1.5 border-t border-[--color-border] pt-3 text-[20px] font-semibold not-italic text-[--color-walnut]' : ''}`}
    >
      <span>{label}</span>
      <span>{value}</span>
    </div>
