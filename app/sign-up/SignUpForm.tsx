...
export default function SignUpForm({ next }: { next?: string }) {
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-4">
      ...
      <label className="flex cursor-pointer items-start gap-2.5 font-serif text-[14px] italic leading-[1.5] text-[--color-ink-muted]">
        <input type="checkbox" required className="mt-0.5 h-[18px] w-[18px] accent-[--color-walnut]" />
        <span>
          I agree to the{' '}
          <Link href={siteConfig.routes.legal.terms} className="not-italic text-[--color-walnut] border-b border-[--color-bronze-deep] pb-px">
            Terms
          </Link>{' '}
          and{' '}
          <Link href={siteConfig.routes.legal.privacy} className="not-italic text-[--color-walnut] border-b border-[--color-bronze-deep] pb-px">
            Privacy Policy
          </Link>
