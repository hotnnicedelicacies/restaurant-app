      ...
      <main>
        <section className="py-[clamp(56px,8vw,96px)]">
          <div className="container">
            <div className="mx-auto max-w-[760px] rounded-[2px] border border-rule bg-cream p-[clamp(36px,5vw,56px)] text-center">
              {cardFailed ? (
                <FailedHeader />
              ) : (
              ...
              <span className="mb-7 inline-block rounded-[2px] border border-rule px-4 py-2.5 font-mono text-[12px] uppercase tracking-[0.2em] text-walnut">
                Order № {order.ref}
              </span>

              <div className="my-6 mx-auto block h-px w-10 bg-bronze opacity-70" />

              <div className="my-7 grid grid-cols-1 gap-6 border-y border-rule py-6 text-left sm:grid-cols-3">
                ...
                <Meta label="When">
                  ...
                  <br /><em className="italic text-ink-muted">Cooked this morning</em>
                </Meta>
                <Meta label={order.paymentMethod === 'card' ? 'Paid' : 'Payment'}>
                  ...
                  {order.paymentMethod === 'card'
                    ...
                    : 'Cash on delivery'}
                  <br />
                  <em className="italic text-ink-muted">
              ...
              {cardPending && (
                <p className="mb-4 mx-auto max-w-[52ch] font-serif text-[14px] italic text-ink-muted">
              ...
              <p className="mt-6 font-serif text-[14px] italic text-ink-muted">
...
function SuccessHeader({ name }: { name: string }) {
  return (
    <>
      <div
        aria-hidden
        className="relative mx-auto mb-6 inline-flex h-[76px] w-[76px] items-center justify-center rounded-full border border-bronze"
      >
        <span className="font-serif text-[36px] italic font-normal leading-none text-bronze-deep">✓</span>
      </div>
      <div className="mb-3 font-mono text-[10px] uppercase tracking-[0.24em] text-bronze-deep">
        Order received · {formatLongDate(new Date())}
      </div>
      <h1 className="m-0 mb-3 font-serif text-[clamp(32px,4.5vw,48px)] font-medium leading-[1.04] tracking-[-0.005em] text-walnut">
        Thank you, <em className="italic font-normal text-bronze-deep">{name}.</em>
      </h1>
      <p className="mx-auto mb-6 max-w-[52ch] font-serif text-[17px] italic leading-[1.5] text-ink-muted">
...
function Meta({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      ...
      <div className="font-serif text-[15px] leading-[1.4] text-walnut">{children}</div>
    </div>
  );
}
